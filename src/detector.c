...
void validate_detector_mapTitan(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou,float iou_thresh)
{
	int j;
	list *options = read_data_cfg(datacfg);
	char *valid_images = option_find_str(options, "valid", "data/train.txt");
...
void find_replace_v2(const char* str, char* orig, char* rep, char* output)
{
...
detection *read_boxes_detection(char *filename,int num_classes)
{
    detection* dets = (detection*)calloc(1, sizeof(detection));
...
// Cambiar múltiples clases
    int classes = 0;
    float *mask = NULL;
    float objectness = 1.0;
// Cambiar múltiples clases
    int sort_class = 0;
    float *prob = (float*)calloc(num_classes,sizeof(float));
    prob[0]=0.0;
    int id;
    int count = 0;
    //printf("%s\n",filename);
    ...
    if(iSpaces==6){
    fscanf(file, "%d %f %f %f %f %f", &id, &x, &y, &w, &h,&c);
} 
   if(iSpaces==5){
    fscanf(file, "%d %f %f %f %f", &id, &x, &y, &w, &h);
}

    while(iSpaces == 6 || iSpaces==5){
        dets = (detection*)realloc(dets, (count + 1) * sizeof(detection));
        box bbox = {x,y,w,h};
        
        dets[count].bbox = bbox;
        dets[count].mask = mask;
        dets[count].objectness = objectness;
        
        dets[count].sort_class = id;
        dets[count].classes = id;
        if(iSpaces==6){
        ...
        dets[count].prob = prob;
        
        ++count;
          if(iSpaces==6){
   iSpaces= fscanf(file, "%d %f %f %f %f %f", &id, &x, &y, &w, &h,&c);
} 
   if(iSpaces==5){
iSpaces=    fscanf(file, "%d %f %f %f %f", &id, &x, &y, &w, &h);
}
    }
    fclose(file);
    return dets;
...
int countlineas(char *filename)
{
    ...
    while(fscanf(file, "%d %f %f %f %f %f", &id, &x, &y, &w, &h,&c) == 6){
...
void validate_detector_map_general(char * pathTXT, char * namesTXT,float iou_thresh)
{
	int j;
	
	char *difficult_valid_images = NULL;
	
	int *map = 0;
	FILE* reinforcement_fd = NULL;
	srand(time(0));

	list *plist = get_paths(pathTXT);
	char **paths = (char **)list_to_array(plist);

	char **paths_dif = NULL;
        list *plist_dif = NULL;
	
	char **names = get_labels(namesTXT);
	...
	const float thresh = .005;
	const float nms = .45;
     /// Depende de lo que nos interese esto se puede ajustar, cuanto más alto más exigente, cuanto más
// bajo menos. 
	//const float iou_thresh = 0.5;
	...
	for (i = nthreads; i < m + nthreads; i += nthreads) {
		...
		for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
			...
			int letterbox = (args.type == LETTERBOX_DATA);
			float hier_thresh = 0;
                        char detectionpath[4096];
                        find_replace_v2(path,"labels","detection",detectionpath);
// Falla nboxes
			detection *dets = read_boxes_detection(detectionpath,classes);
                        //nboxes = countlines(detectionpath);
 //printf("tam: %d\n",nboxes);
			//detection *dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes, letterbox); // for letterbox=1
			...
			int num_labels = 0;
			box_label *truth = read_boxes(path, &num_labels);
			int i, j;
			for (j = 0; j < num_labels; ++j) {
				truth_classes_count[truth[j].id]++;
			}
			...
			const int checkpoint_detections_count = detections_count;
//nboxes = countlineas(detectionpath);

// Falla nboxes

//printf(sizeof(5));
//printf(sizeof(countlineas(detectionpath)));
//printf("Hola\n");
                        //printf("lineas %d\n",countlineas(detectionpath));
			for (i = 0; i < countlineas(detectionpath); ++i) {
                                //printf("Hola\n");
				int class_id;
				for (class_id = 0; class_id < classes; ++class_id) {
	...
	for (i = 0; i < classes; ++i) {
		...
		avg_precision = avg_precision / 11;//11
                // Cambiar múltiples clases0
		printf("class_id = %d, name = %d, \t ap = %2.2f %% \n", i,i,  avg_precision*100);
		mean_average_precision += avg_precision;
	}
	...
	const float cur_recall = (float)tp_for_thresh / ((float)tp_for_thresh + (float)(unique_truth_count - tp_for_thresh));
	const float f1_score = 2.F * cur_precision * cur_recall / (cur_precision + cur_recall);
	printf(" precision = %1.2f, recall = %1.2f, F1-score = %1.2f \n",
		 cur_precision, cur_recall, f1_score);

	printf("  TP = %d, FP = %d, FN = %d, average IoU = %2.2f %% \n", 
...
void run_detector(int argc, char **argv)
{
    ...
    else if (0 == strcmp(argv[2], "compare")) validate_detector_map_general(datacfg,cfg,iou_thresh);
    else if (0 == strcmp(argv[2], "valid")) validate_detector(datacfg, cfg, weights, outfile);
    else if (0 == strcmp(argv[2], "recall")) validate_detector_recall(datacfg, cfg, weights);
    else if (0 == strcmp(argv[2], "map")) validate_detector_mapTitan(datacfg, cfg, weights, thresh,iou_thresh);
