...
# 
# ```python pascal2yolo -d <dir> -f <filenames>```
# 
...
def boxesFromPascalVOC(labelPath):
    ...
    for object in objects:
        ...
        c = float(object.find('confidence').text)
        boxes.append((category, (x, y, w, h),c))
    return boxes
...
def transformPascalVOCBox2YOLO(wI,hI,box,lines):
    ...
    w = float(w)
    h = float(h)
    return (clase, (x+w/2)/wI, (y+h/2)/hI,w/wI,h/hI,c)    
...
def transformPascalVOCFile2YOLO(imagePath,outputPath,lines):
...
ap.add_argument("-f", "--filenames", required=True,help="path to the file of names")
...
fo = open(namesPath, "r")
lines = fo.readlines()
lines = [line[0:line.rfind("\n")] for line in lines]


img_files = list(paths.list_files(datasetPath, validExts=(".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".tif")))
for img_file in img_files:
    transformPascalVOCFile2YOLO(img_file,datasetPath,lines)
